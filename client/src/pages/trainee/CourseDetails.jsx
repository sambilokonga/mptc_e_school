...
const CourseDetails = () => {
  ...
  return courseData ? (
    <>
    <div className='relative flex flex-col-reverse md:flex-row gap-10  items-start justify-between md:px-36 px-8 pt-20 md:pt-30 text-sm'>
        <div className='absolute top-0 left-0 w-full h-section-height -z-1 bg-gradient-to-b from-cyan-100/70'></div>
      {/* left colmn */}
      <div className='max-w-xl z-100 text-gray-500'>
        <h1 className='md:text-course-details-heading-large text-course-details-small font-semibold text-gray-800'>{courseData.courseTitle}</h1>
        <p className='pt-4 text-sm md:text-base' dangerouslySetInnerHTML={{__html:courseData.courseDescription.slice(0,200)}}></p>
        {/* review and rating /first update coursecard and copy from there */}
        <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
              <p>{calculateRating(courseData)}</p>
              <div className='flex '>
                {[...Array(5)].map((_, i)=>(
                    <img src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} alt="" />
                ))}
              </div>
            ...
            <div className='py-20 text-sm md:text-default'>
              <h3 className='text-xl text-semibold text-gray-800'>Course Description</h3>
