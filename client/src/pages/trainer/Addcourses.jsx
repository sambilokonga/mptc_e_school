...
const Addcourses = () => {
 ...
 useEffect(()=>{
  ...
  return (
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between p-4 md:p-8 md:pb-0 pt-8 pt-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course title</p>
          <input onChange={e=> setCourseTitle(e.target.value)} value={courseTitle} required className='outline-none py-2 md:py-2.5 px-3 rounded border border-gray-500' type="text" placeholder='Type here' />
        </div>
        <div className='flex flex-col gap-1'>
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input onChange={e=> setCoursePrice(e.target.value)} value={coursePrice} required type="number" placeholder='0' className='outline-none py-2 md:py-2.5 w-28 px-3 rounded border border-gray-500' />
          </div>
          <div className='flex flex-col md:flex-row items-center gap-3'>
            <p>Course Thumbnail</p>
            <label htmlFor="Thumbnail image" className='flex items-center gap-3'>
        ...
        <div className='flex flex-col gap-1'>
          <p>Discount %</p>
          <input onChange={e=> setDiscount(e.target.value)} value={discount} type="number" placeholder='0' min={0} max={100} required className='outline-none py-2 md:py-2.5 w-28 px-3 rounded border border-gray-500 ' />
        </div>
        ...
        <div>
          ...
          <div onClick={()=>handleChapter("add")} className='flex items-center justify-center bg-blue-100 p-2 rounded-lg cursor-pointer'>+ Add Chapter</div>
          {showPopup && (
            <div className='fixed inset-0 flex items-center justify-center bg-gray-500/50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                ...
                <div className='mb-2 flex items-center gap-2'>
                  ...
                  onChange={(e)=> setLectureDetails({...lectureDetails, isPreviewFree: e.value.target})}
